...
日常的な運用手順は **[docs/運用メモ.md](docs/運用メモ.md)** に記載されている。ユーザーから「SEO編集の手順を実行して」「新商品の反映」のような曖昧な指示を受けた際は、まずこのファイルを参照して該当手順に従うこと。

現在メモに含まれる手順:
- SEO項目の一括編集・反映（CSV経由）
- 新商品のカラーミー登録後にシートに反映する
- 仕入れ先URL（J列）の自動補完
...
**A列(採用フラグ)の値**:
- `採用` : カラーミー登録対象。価格変動も継続追跡
- `未採用` : 採用見送り（再検討可）。スクレイピングでは新規追加されない（URL重複検出）が、価格再取得対象でもない
- `検討中` : 判断保留。未採用と同等の扱い
- `NG` : スクレイパーが自動的に除外パターン（OGP BOX/MintDirect等）を検出した場合の自動マーク
- `除外` : ユーザーが明示的に除外宣言。`get_adopted_urls_from_spreadsheet` で明示的にスキップされ、価格再取得や採用登録の対象外。**行は残るので再スクレイピングで復活しない**
- `作業完了` : 旧フラグ。視覚的な作業進捗マーカー（色分け確認用、機能影響なし）
- `保留` : 採用も視野に入れているが他商品との兼ね合いで一時保留
