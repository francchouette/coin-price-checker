#!/bin/bash
# ================================================================
# ブリオンスター商品取得 定期実行スクリプト
#
# 実行内容:
#   Bullionstar APIから商品一覧を取得し、価格・在庫をスクレイピング
#   → ブリオンスター商品ページ一覧シートに保存
#
# 使用方法:
#   手動実行: bash scripts/bs-scrape.sh
#   launchd:  自動実行（6時間ごと）
# ================================================================
...
LOG_FILE="${LOG_DIR}/bs-scrape-${TIMESTAMP}.log"
...
if "${PYTHON}" -m src.bullionstar_products --fetch-prices >> "${LOG_FILE}" 2>&1; then
    ...
    EXIT_CODE=$?
    log ""
    log "ERROR: ブリオンスター商品取得が失敗しました (exit code: ${EXIT_CODE})"
    log "詳細: ${LOG_FILE}"
    exit 1
fi

# --- 古いログの削除（30日以上前）---
find "${LOG_DIR}" -name "bs-scrape-*.log" -mtime +30 -delete 2>/dev/null || true
