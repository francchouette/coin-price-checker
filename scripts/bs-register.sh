#!/bin/bash
# ================================================================
# ブリオンスター採用商品 カラーミー登録スクリプト
#
# 実行内容:
#   ブリオンスター商品ページ一覧で A列=「採用」の商品を
#   カラーミーAPIで自動登録
#
# 使用方法:
#   手動実行: bash scripts/bs-register.sh
...
LOCK_FILE="/tmp/bs-register.lock"
...
LOG_FILE="${LOG_DIR}/bs-register-${TIMESTAMP}.log"
...
cd "${PROJECT_DIR}"
log "=========================================="
log "ブリオンスター採用商品 カラーミー登録 開始"
log "=========================================="

START=$(date +%s)

if "${PYTHON}" -m src.register_adopted_products --source bs --upload-images --verbose >> "${LOG_FILE}" 2>&1; then
    END=$(date +%s)
    ELAPSED=$(( END - START ))
    log ""
    log "=========================================="
    log "ブリオンスター採用商品 カラーミー登録 完了"
...
# --- 古いログの削除（30日以上前）---
find "${LOG_DIR}" -name "bs-register-*.log" -mtime +30 -delete 2>/dev/null || true
