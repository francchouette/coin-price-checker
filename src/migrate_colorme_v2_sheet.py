...
新構造（86列）のインデックス:
  ...
  44-47 (AS-AV): 大カテゴリーID〜型番
  48-54 (AW-BC): 在庫数〜単位
  55-58 (BD-BG): 個別送料〜配送不要
  59-62 (BH-BK): 商品説明〜備考
  63-72 (BL-BU): メイン画像URL〜画像URL8
  73-75 (BV-BX): ページタイトル〜メタキーワード
  76-80 (BY-CC): 軽減税率対象〜利用不可決済
  81-82 (CD-CE): 掲載開始日時〜掲載終了日時
  83-85 (CF-CH): 同期日時〜商品更新日時
"""
...
def migrate_row(old_row: list) -> list:
    ...
    #
    # 旧データ位置（old_row[15]〜）を新位置（new_row[20]〜）にコピー
...
def main():
    ...
    try:
        ...
        # ヘッダーの確認
        if len(headers) != 86:
            logger.warning(f"ヘッダーが86列ではありません: {len(headers)}列")
