...
class Config:
    ...
    # ブリオンスター商品ページ一覧シートのヘッダー（38列: A-AL）
    # ※商品仕入れ先一覧シートと同じ列構造
    BULLIONSTAR_PRODUCT_HEADERS: list[str] = [
        "仕入れ先商品ID",      # A列: BS-XXXXXX形式
        "仕入れ先商品名",      # B列
        "仕入れ先商品URL",     # C列: ユニークキー
        "仕入れ先サイト",      # D列: 固定値「Bullionstar」
        "最上位カテゴリ",      # E列
        "親カテゴリ",          # F列
        "子カテゴリ",          # G列
        "ロケーション",        # H列
        "初回取得日",          # I列
        "商品グループID",      # J列: 将来用
        "在庫状況",            # K列: In Stock/Out of Stock
        "現在価格（現地通貨）", # L列: スクレイピング
        "取引通貨",            # M列: USD/SGD等
        "為替種類",            # N列: Wise/クレカ
        "為替レート",          # O列: 自動取得
        "日本円換算価格",      # P列: =L*O
        ...
        "画像URL7",            # AF列
        "画像URL8",            # AG列
        "仕様・スペック",      # AH列: 重量、純度、サイズ等
        "商品説明（英語）",    # AI列: 仕入れ先の商品説明
        "商品説明（日本語）",  # AJ列: AI翻訳後の説明
        "発行年",              # AK列: 発行年/ミント年
        "発行数・限定数",      # AL列: ミンテージ情報
    ]
    ...
    # 商品仕入れ先一覧シートのヘッダー（38列: A-AL）
    # ※ブリオンスター商品ページ一覧シートと同じ列構造
