...
class Config:
    ...
    # ブリオンスター商品ページ一覧シートのヘッダー（76列: A-BX）
    # ※A-C列: 管理列（採用フラグ、登録状況、仕入れ先商品ID）
    # ※D-BX列: 商品情報・カラーミー登録用項目
    BULLIONSTAR_PRODUCT_HEADERS: list[str] = [
        ...
        "小カテゴリーID",          # AO列: カラーミー小カテゴリーID（自動）
        "グループID",              # AP列: カラーミーグループID（自動、カンマ区切り）
        "型番",                    # AQ列: 自動=仕入れ先商品ID

        # === 在庫管理（AR-AX列: 7列）===
        "在庫数",                  # AR列: 初期在庫数（手入力、デフォルト10）
        "在庫管理",                # AS列: 「する」「しない」
        "残りわずか数",            # AT列: few_num（手入力、デフォルト3）
        "売切れ表示",              # AU列: 「表示」「非表示」
        "最小購入数",              # AV列: min_num（手入力、デフォルト1）
        "最大購入数",              # AW列: max_num（手入力、デフォルト0）
        "単位",                    # AX列: unit（手入力、空欄可）

        # === 送料・配送（AY-BB列: 4列）===
        "個別送料",                # AY列: delivery_charge（手入力、デフォルト0）
        "クール便料金",            # AZ列: cool_charge（手入力）
        "重量(g)",                 # BA列: weight（手入力、空欄可）
        "配送不要",                # BB列: 「不要」「必要」

        # === 商品説明（BC-BF列: 4列）===
        "商品説明",                # BC列: カラーミー用商品説明（AIで生成）
        "簡易説明",                # BD列: カラーミー用簡易説明（AIで生成）
        "スマホ説明",              # BE列: スマホ用説明（AIで生成）
        "備考",                    # BF列: 備考（手入力）
        ...
        "メタディスクリプション",  # BR列: meta_description（AIで生成）
        "メタキーワード",          # BS列: meta_keywords（AIで生成）

        # === フラグ・設定（BT-BX列: 5列）===
        "軽減税率対象",            # BT列: 「対象」「対象外」
        "デジタルコンテンツ",      # BU列: 「対象」「対象外」
        "定期購入",                # BV列: 「対象」「対象外」
        "表示順",                  # BW列: sort（手入力、デフォルト0）
        "利用不可決済",            # BX列: カンマ区切りの決済ID（手入力）

        # === 掲載期間（BY-BZ列: 2列）===
        "掲載開始日時",            # BY列: 掲載開始日時（手入力、空欄可）
        "掲載終了日時",            # BZ列: 掲載終了日時（手入力、空欄可）

        # === システム情報（CA-CC列: 3列）===
        "同期日時",                # CA列: 最終同期日時
        "商品作成日時",            # CB列: カラーミー側の作成日時
        "商品更新日時",            # CC列: カラーミー側の更新日時
    ]
