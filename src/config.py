...
class Config:
    ...
    BULLIONSTAR_PRODUCT_HEADERS: list[str] = [
        ...
        "仕入れ先商品名",      # D列
        "仕入れ先商品URL",     # E列: ユニークキー
        "仕入れ先サイト",      # F列: 固定値「Bullionstar」（自動）
        "最上位カテゴリ",      # G列
        "親カテゴリ",          # H列
        "子カテゴリ",          # I列
        "製造国",              # J列: サイトからスクレイピング
        "初回取得日",          # K列: 自動
        "在庫状況",            # L列: In Stock/Out of Stock（サイトからスクレイピング）
        "現在価格（現地通貨）", # M列: スクレイピング
        "取引通貨",            # N列: USD/SGD等（サイトからスクレイピング）
        "為替種類",            # O列: Wise/クレカ（手入力）
        "為替レート",          # P列: 自動取得
        "日本円換算価格",      # Q列: =M*P
        "カラーミー商品ID",    # R列: 登録後自動設定
        # 画像URL（S-AB列: 10列）
        "画像URL1",            # S列
        "画像URL2",            # T列
        "画像URL3",            # U列
        ...
        "画像URL10",           # AB列
        # 仕入れ先詳細情報（AC-AE列: 3列）
        "仕様・スペック",      # AC列: 重量、純度、サイズ等
        "商品説明（英語）",    # AD列: 仕入れ先の商品説明
        "商品説明（日本語）",  # AE列: AI翻訳後の説明
        # 以下はAK→AF, AL→AGに繰り上げ（発行年・発行数は後で追加予定）
        # → 後続ブロックに入れる
        "発行年",              # AF列: 発行年/ミント年（サイトからスクレイピング）
        "発行数・限定数",      # AG列: ミンテージ情報（サイトからスクレイピング）

        # === カラーミー登録用項目（AH-CE列: 40列）===
        # ※新カラーミー商品管理シートの列順序に準拠
        # ※CM商品名を追加（商品名とページタイトルを別管理）

        # C. カラーミー商品名（AH列: 1列）- 新規追加
        "CM商品名",            # AH列: カラーミー用商品名（AI翻訳、手入力可）

        # D. 価格計算（AI-AT列: 12列）
        "為替レート(CM)",      # AI列: =P列
        "仕入れ額(日本円)",    # AJ列: =Q列（日本円換算価格）
        "枚数",                # AK列: セット枚数（手入力、デフォルト1）
        "仕入れ合計",          # AL列: 計算式 =AJ*AK
        "設定マージン率",      # AM列: 利益率（手入力、デフォルト1.1=10%）
        "設定マージン額",      # AN列: 固定マージン額（手入力）
        "送料",                # AO列: 仕入れ送料（手入力）
        "手数料",              # AP列: その他手数料（手入力）
        "合計原価",            # AQ列: 計算式 =AL+AO+AP
        "適正価格",            # AR列: 計算式 =AQ*AM+AN
        "粗利額",              # AS列: 計算式 =AR-AQ
        "粗利率",              # AT列: 計算式 =AS/AR*100

        # E. カラーミー価格情報（AU-AZ列: 6列）
        "販売価格",            # AU列: 計算式 =AR
        "定価",                # AV列: 定価（手入力）- 空欄時は販売価格と同じ
        "会員価格",            # AW列: 会員価格（手入力）
        "原価",                # AX列: 計算式 =AQ
        "消費税込販売価格",    # AY列: 計算式 =AU*1.1
        "消費税額",            # AZ列: 計算式 =AU*0.1

        # F. カテゴリー・グループ（BA-BD列: 4列）- Add Productロジックで自動設定
        "CM大カテゴリーID",    # BA列: カラーミー大カテゴリーID（自動）
        "CM小カテゴリーID",    # BB列: カラーミー小カテゴリーID（自動）
        "CMグループID",        # BC列: カラーミーグループID（自動、カンマ区切り）
        "型番",                # BD列: 自動=仕入れ先商品ID

        # G. 在庫管理（BE-BK列: 7列）
        "在庫数",              # BE列: 初期在庫数（手入力、デフォルト10）
        "在庫管理",            # BF列: 「する」「しない」（手入力、デフォルト「する」）
        "残りわずか数",        # BG列: few_num（手入力、デフォルト3）
        "売切れ表示",          # BH列: 「表示」「非表示」（手入力、デフォルト「表示」）
        "最小購入数",          # BI列: min_num（手入力、デフォルト1）
        "最大購入数",          # BJ列: max_num（手入力、デフォルト0=無制限）
        "単位",                # BK列: unit（手入力、空欄可）

        # H. 送料・配送（BL-BO列: 4列）
        "個別送料",            # BL列: delivery_charge（手入力、デフォルト0）
        "クール便料金",        # BM列: cool_charge（手入力）
        "重量(g)",             # BN列: weight（手入力、空欄可）
        "配送不要",            # BO列: 「不要」「必要」（手入力、デフォルト「必要」）

        # I. 商品説明（BP-BR列: 3列）- AIで生成
        "CM商品説明",          # BP列: カラーミー用商品説明（AIで生成）
        "CM簡易説明",          # BQ列: カラーミー用簡易説明（AIで生成）
        "CMスマホ説明",        # BR列: スマホ用説明（AIで生成）

        # J. SEO項目（BS-BU列: 3列）- AIで生成、※Playwrightで登録
        "ページタイトル",      # BS列: page_title（AIで生成）
        "メタディスクリプション", # BT列: meta_description（AIで生成）
        "メタキーワード",      # BU列: meta_keywords（AIで生成）

        # K. フラグ・設定（BV-BZ列: 5列）
        "軽減税率対象",        # BV列: 「対象」「対象外」（手入力、デフォルト「対象外」）
        "デジタルコンテンツ",  # BW列: 「対象」「対象外」（手入力、デフォルト「対象外」）
        "定期購入",            # BX列: 「対象」「対象外」（手入力、デフォルト「対象外」）
        "表示順",              # BY列: sort（手入力、デフォルト0）
        "利用不可決済",        # BZ列: カンマ区切りの決済ID（手入力）

        # L. 掲載期間（CA-CB列: 2列）
        "掲載開始日時",        # CA列: 掲載開始日時（手入力、空欄可）
        "掲載終了日時",        # CB列: 掲載終了日時（手入力、空欄可）

        # M. システム情報（CC-CE列: 3列）- 自動
        "登録日時",            # CC列: カラーミー登録日時
        "商品作成日時",        # CD列: カラーミー側の作成日時
